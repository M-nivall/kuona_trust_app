...
//        user=session.getUserDetails();
        ...
        gotostaff.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent in = new Intent(getApplicationContext(), StaffLogin.class);
                startActivity(in);
            }
        });
                        ...
                        try {
                            ...
                            if (status.equals("1")) {
                                ...
                                for (int i = 0; i < jsonArray.length(); i++) {
                                    ...
                                    String bio = jsn.getString("bio");

                                    session.loginUser(clientID, firstname, lastname, username, phoneNo, email, dateCreated, user_type);

                                    Intent intent;
                                    if (bio == null || bio.isEmpty() || bio.equalsIgnoreCase("NULL") && user_type.equals("Artist")) {
                                        // Redirect to complete profile
                                        intent = new Intent(getApplicationContext(), CompleteProfile.class);
                                    } else {
